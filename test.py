...
from test_tomcat import predict_logits_text_first_with_tomcat
...
if __name__ == "__main__":
    ...
    if config.use_wandb:
        ...
        wandb.init(project='TTA-' + config.dataset)
    # set the seed value
    def set_seed(seed):
        np.random.seed(seed)
        random.seed(seed)
    ...
    if (hasattr(config, 'text_first') and config.text_first):
        print('text first')
        if config.use_tta:
            predict_logits_func = predict_logits_text_first_with_tomcat
        else:
