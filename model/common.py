...
class CustomTextEncoder(torch.nn.Module):
    def __init__(self, clip_model, tokenizer, dtype=torch.float16, is_copy=False):
        super().__init__()
        self.dtype = dtype
        ...
        if is_copy:
            self.token_embedding = copy.deepcopy(clip_model.token_embedding)
        else:
    ...
    def forward(self, token_ids, token_tensors, enable_pos_emb):
        ...
        bs = x.shape[0]
        seq = torch.arange(bs)
        max_pos = token_ids.argmax(dim=-1)
        tf = (
            x[
                seq, max_pos
            ]  # POS of <EOS>
